"""FROZEN copy of the pre-2026-08-13 key logic. Do not edit. Do not import
from canonical.py.

state/seen.json holds thousands of keys written in the old formats, which had
three bugs that merged unrelated jobs (see canonical.py): Workday and iCIMS
keys dropped the tenant, the URL fallback dropped the whole query string, and
the identity key erased season and level. Fixing those changes what a key looks
like, so every stored key would suddenly miss and the next digest would re-send
the entire backlog.

src/migrate.py bridges that by mapping old keys to new ones. To do it safely it
needs to compute an old key *exactly* as the code that wrote state/seen.json
did -- so this is a verbatim copy rather than a flag on canonical.py, because a
flag could be broken by a later edit without anyone noticing. tests/
test_migration.py pins the output against a golden file for the same reason.

Delete this module (and dedup.legacy_keys_until in config/sources.yaml) once
the legacy read window has closed.
"""
...
def legacy_keys_for(job) -> list[str]:
    """The keys this job WOULD have had under the old scheme."""
